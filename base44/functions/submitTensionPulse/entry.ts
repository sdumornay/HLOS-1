...
export default async function(req) {
  try {
    ...
    // Enforce stage gating: Tension Pulse is a Stage 1 (Stabilize) assessment
    const access = await validateStageAccess(base44, orgId, 'tension_pulse');
    if (!access.allowed) {
      return Response.json({ error: access.reason }, { status: 403 });
    }

    const record = await base44.asServiceRole.entities.TensionPulse.create({
      ...scores,
